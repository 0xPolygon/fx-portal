...
interface IFxMintableERC1155 is IERC1155 {
    ...
    function initialize(address _fxManager, address _connectedToken, string calldata uri, address minter) external;

    function connectedToken() external returns (address);

    function mintToken(address user, uint256 id, uint256 amount, bytes calldata data) external;
