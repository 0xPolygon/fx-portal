...
contract FxERC1155ChildTunnel is FxBaseChildTunnel, Create2, ERC1155Holder {
	...
	function withdrawToBatch
		(address childToken, address receiver, uint256[] memory ids, uint256[] memory amounts, bytes memory data)
		public {
	...
	function _withdrawBatch
		...
		bytes memory message = abi.encode(WITHDRAW_BATCH, abi.encode(rootToken, childToken, receiver, ids, amounts, data));
		 _sendMessageToRoot(message);
	}
