...
import { IFxERC721URIStorage as IFxERC721 } from '../../tokens/IFxERC721URIStorage.sol';
...
contract FxERC721URIStorageChildTunnel is FxBaseChildTunnel, Create2, IERC721Receiver {
    ...
    function _syncDeposit(bytes memory syncData) internal {
        (address rootToken, address depositor, address to, uint256 tokenId, string memory _tokenURI, bytes memory depositData) = abi.decode(syncData, (address, address, address, uint256, string, bytes));
        address childToken = rootToChildToken[rootToken];

        // deposit tokens
        IFxERC721 childTokenContract = IFxERC721(childToken);
        childTokenContract.mint(to, tokenId, _tokenURI, depositData);
    }
