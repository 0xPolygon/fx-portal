...
describe("FxERC721", () => {
  ...
  it("setmetadata:fail", async () => {
    await expect(
      fxERC721.connect(other).setupMetaData("New Name", "New Symbol")
  ...
  it("mint:fail", async () => {
    await expect(
      fxERC721.connect(other).mint(await wallet.getAddress(), 1, "0x")
    ).to.be.revertedWith("Invalid sender");
  });

  it("burn:fail", async () => {
    await expect(fxERC721.connect(other).burn(0)).to.be.revertedWith(
      "Invalid sender"
    );
  });
  ...
  it("transfer:fail", async () => {
    await expect(
      fxERC721["safeTransferFrom(address,address,uint256)"](
        await wallet.getAddress(),
        await other.getAddress(),
        1
      )
  ...
  it("safeTransferFrom", async () => {
    ...
    expect(await fxERC721.getApproved(0)).to.eq(
