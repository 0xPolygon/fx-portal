...
describe('FxERC20', () => {
  ...
  it('name, symbol, decimals, totalSupply, balanceOf', async () => {
    ...
    expect(await fxERC20.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY);
  });
  ...
  it('initialize:fail', async () => {
    await expect(fxERC20.initialize(await other.getAddress(), fxERC20ChildTunnel.address, "New Name", "New Symbol", 18))
      .to.be.revertedWith('Token is already initialized');
  })

  it('setmetadata:fail', async () => {
    await expect(fxERC20.connect(other).setupMetaData("New Name", "New Symbol", 18))
  ...
  it('mint:fail', async () => {
    await expect(fxERC20.connect(other).mint(await wallet.getAddress(), TOTAL_SUPPLY))
      .to.be.revertedWith('Invalid sender');
  })

  it('burn:fail', async () => {
    await expect(fxERC20.connect(other).burn(await other.getAddress(), TOTAL_SUPPLY))
      .to.be.revertedWith('Invalid sender');
  })
  ...
  it('approve', async () => {
    ...
    expect(await fxERC20.allowance(await wallet.getAddress(), await other.getAddress())).to.eq(TEST_AMOUNT);
  });
  ...
  it('transfer', async () => {
    await expect(fxERC20.transfer(await other.getAddress(), TEST_AMOUNT))
      ...
      .withArgs(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT);
    expect(await fxERC20.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY.sub(TEST_AMOUNT));
    expect(await fxERC20.balanceOf(await other.getAddress())).to.eq(TEST_AMOUNT);
  });

  it('transfer:fail', async () => {
    await expect(fxERC20.transfer(await other.getAddress(), TOTAL_SUPPLY.add(1))).to.be.reverted; // ds-math-sub-underflow
    await expect(fxERC20.connect(other).transfer(await wallet.getAddress(), 1)).to.be.reverted; // ds-math-sub-underflow
  });

  it('transferFrom', async () => {
    await fxERC20.approve(await other.getAddress(), TEST_AMOUNT);
    await expect(fxERC20.connect(other).transferFrom(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT))
      .to.emit(fxERC20, 'Transfer')
      .withArgs(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT);
    expect(await fxERC20.allowance(await wallet.getAddress(), await other.getAddress())).to.eq(0);
    expect(await fxERC20.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY.sub(TEST_AMOUNT));
    expect(await fxERC20.balanceOf(await other.getAddress())).to.eq(TEST_AMOUNT);
  });

  it('transferFrom:max', async () => {
    await fxERC20.approve(await other.getAddress(), ethers.constants.MaxUint256);
    await expect(fxERC20.connect(other).transferFrom(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT))
      .to.emit(fxERC20, 'Transfer')
      .withArgs(await wallet.getAddress(), await other.getAddress(), TEST_AMOUNT);
    expect(await fxERC20.allowance(await wallet.getAddress(), await other.getAddress())).to.eq(
      ethers.constants.MaxUint256.sub(TEST_AMOUNT),
    );
    expect(await fxERC20.balanceOf(await wallet.getAddress())).to.eq(TOTAL_SUPPLY.sub(TEST_AMOUNT));
    expect(await fxERC20.balanceOf(await other.getAddress())).to.eq(TEST_AMOUNT);
  });
