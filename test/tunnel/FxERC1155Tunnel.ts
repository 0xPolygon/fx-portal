...
describe('FxERC1155Tunnel', () => {
  ...
  it('fxRoot, mapToken', async () => {
    let childTokenMap = await fxERC1155ChildTunnel.rootToChildToken(fxERC1155.address);
    let rootTokenMap = await fxERC1155RootTunnel.rootToChildTokens(fxERC1155.address);
    ...
    childTokenMap = await fxERC1155ChildTunnel.rootToChildToken(fxERC1155.address);
    rootTokenMap = await fxERC1155RootTunnel.rootToChildTokens(fxERC1155.address);
    expect(childTokenMap).to.eq(rootTokenMap);
  ...
  it('fxRoot, deposit with mapToken', async () => {
    ...
    await expect(fxERC1155RootTunnel.deposit(fxERC1155.address, await wallet.getAddress(), tokenId, tokenAmount, "0x"))
      .to.emit(fxERC1155RootTunnel, 'TokenMappedERC1155')
      .to.emit(fxERC1155ChildTunnel, 'TokenMapped')
    ...
    const childFxERC1155: FxERC1155 = await new FxERC1155__factory(wallet).attach(childTokenMap);
    expect(await childFxERC1155.balanceOf(await wallet.getAddress(), tokenId)).to.eq(tokenAmount);
  })
  ...
  it('fxRoot, deposit after mapToken', async () => {
    ...
    await expect(fxERC1155RootTunnel.deposit(fxERC1155.address, await wallet.getAddress(), tokenId, tokenAmount, "0x"))
      .to.emit(fxERC1155RootTunnel, 'FxDepositERC1155')
      .withArgs(fxERC1155.address, await wallet.getAddress(), await wallet.getAddress(), tokenId, tokenAmount)
      .to.emit(childFxERC1155, 'TransferSingle')
      .withArgs(fxERC1155ChildTunnel.address, "0x0000000000000000000000000000000000000000", await wallet.getAddress(), tokenId, tokenAmount);
  ...
  it('fxChild, withdraw fail: unmapped token', async() => {
    ...
    await expect(fxERC1155ChildTunnel.withdraw(fxERC1155.address, tokenId, tokenAmount, "0x")).to.be.revertedWith(
  ...
  it('fxChild, withdraw success on child chain', async() => {
    ...
    const WITHDRAW = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("WITHDRAW"));

    const abiCoder = ethers.utils.defaultAbiCoder;
    const messageData = abiCoder.encode(['address', 'address', 'address', 'uint256', 'uint256', 'bytes'], [fxERC1155.address, childFxERC1155.address, await wallet.getAddress(), tokenId, tokenAmount, "0x"]);
    const withdrawMessageData = abiCoder.encode(['bytes32', 'bytes'], [WITHDRAW, messageData]);
  ...
  it('fxChild, withdraw success syncing with root chain', async() => {
    ...
    const withdrawTx = await fxERC1155ChildTunnel.withdraw(childFxERC1155.address, tokenId, tokenAmount, "0x");
